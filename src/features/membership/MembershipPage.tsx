        ...
        {plans.map((plan) => (
          <article key={plan.id} className="membership-tier">
            ...
            <button
              ...
              disabled={profile?.membershipPlan === plan.id}
              style={profile?.membershipPlan === plan.id ? { opacity: 0.5, cursor: 'default' } : {}}
            >
              {profile?.membershipPlan === plan.id ? 'DIN PLAN' : 'VÄLJ NIVÅ'}
            </button>
          </article>
        ))}
      </section>
